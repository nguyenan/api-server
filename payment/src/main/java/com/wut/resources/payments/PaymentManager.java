...
//	private PaymentProvider braintree = new StripeProvider(); // TODO make BrainTreeProvider();
	...
	public PaymentProvider getPaymentProcessor(String processorName, String customer) {
		if (processorName.equals("stripe")) {
			String apiKey = SettingsManager.getClientSettings(customer, "stripe-api-key");
			return new StripeProvider(apiKey);
		} else if (processorName.equals("braintree")) {
			String merchantId = SettingsManager.getClientSettings(customer, "braintree-mechant-id");
			String publicKey = SettingsManager.getClientSettings(customer, "braintree-public-key");
			String privateKey = SettingsManager.getClientSettings(customer, "braintree-private-key");
