...
public abstract class TemplateOperation extends AbstractOperation {
	...
	protected String getClientCodeDirectory(WutRequest request) {
		...
		String branch = SettingsManager.getClientSettings(customerId, "template.git.branch");
		String repository = SettingsManager.getClientSettings(customerId, "template.git.repository");
		String adminRepository = SettingsManager.getSystemSetting("default.git.repository.admin");
		if (branch.isEmpty() || repository.isEmpty())
			throw new SettingNotFoundException("template.git.branch, template.git.repository");
		if (repository.equals(adminRepository))
			return String.format("%s%s%s", SettingsManager.getSystemSetting("code.dir"), "admin/", branch);
		else
	...
	// String clientCodeDirectory = getClientCodeDirectory(request);
	// String gitPath = getGitPath();
	// SystemHelper.runCommand(clientCodeDirectory, gitPath, new String[] {
	// "pull" }, null);
	// }

	// private String getOutputDirectory(WutRequest request) throws
	...
	// }
	//
	// protected String getOutputFilePath(WutRequest request) throws
	// MissingParameterException {
	...
	protected String[] getPhantomParameters(WutRequest request) throws MissingParameterException {
		...
		String templateLocationWithParams = templateLocation
	...
	protected String getUrlParametersString(WutRequest request) throws MissingParameterException {
